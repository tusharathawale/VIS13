...
// **********************************************************
  ...
  /// change!
  /// Compute expected level-crossing location and its variance
  /// @param PTYPE = Precision type to be used in calculations. Should be float of double.
  template <class PTYPE, class DTYPE, class STYPE, class VTYPE,
	    class CTYPE0, class CTYPE1, class CTYPE2>
   ...
   const VTYPE isovalue, CTYPE2 coord2, const STYPE s2, const STYPE s3, float* max_variance, float* color)
  {
    ...
    double lin;
    STYPE s_diff = s1 - s0;
    const PTYPE EPSILON = 0.000000001;
    
    alpha_distribution ad;      
    z_density_uniform uni;		
    piecewise pic;  
    double expected, c_prob, sig;

    uni.setNumUniformInKde1(1);	
    uni.setNumUniformInKde2(1);	
    
    // Get expected level-crossing location and variance
    if (s_diff > EPSILON || s_diff < -EPSILON) {  
	...
	{	
	  if ((s2 > EPSILON) && (s3 > EPSILON))	
	    {	      
	      ...
	      w0 = uni.kde_z_pdf_expected(&s1, s3, &s0, s2, isovalue);

	      sig = uni.kde_z_pdf_variance(&s1, s3, &s0, s2, isovalue, w0);		

	    }
	  else if ((s2<=EPSILON) && (s3>EPSILON))
	    {

	      w0 = uni.kde_z_pdf_expected(&s1, s3, &s0, EPSILON, isovalue);

	      sig = uni.kde_z_pdf_variance(&s1, s3, &s0, EPSILON, isovalue, w0);		

	    }
	  else if ((s2 > EPSILON) && (s3 <= EPSILON))	
	    {	

	      w0 = uni.kde_z_pdf_expected(&s1, EPSILON, &s0, s2, isovalue);	

	      sig = uni.kde_z_pdf_variance(&s1, EPSILON, &s0, s2, isovalue, w0); 

	    }
	  else
	    {

	      w0 = (double)((s1 - isovalue) / s_diff);
	      sig = 0;
        ...
        else
	{
	  if ((s2 > EPSILON) && (s3 > EPSILON))	
	    {	
	      ...
	      w0 = uni.kde_z_pdf_expected(&s0, s2, &s1, s3, isovalue);	

	      sig = uni.kde_z_pdf_variance(&s0, s2, &s1, s3, isovalue, w0); 		

	    }
	  else if ((s2 <= EPSILON) && (s3 > EPSILON))	
	    {	
	      
	      w0 = uni.kde_z_pdf_expected(&s0, EPSILON, &s1, s3, isovalue);		

	      sig = uni.kde_z_pdf_variance(&s0, EPSILON, &s1, s3, isovalue, w0); 	

	    }
	  else if ((s2 > EPSILON) && (s3 <= EPSILON))	
	    {	

	      w0 = uni.kde_z_pdf_expected(&s0, s2, &s1, EPSILON, isovalue);	

	      sig = uni.kde_z_pdf_variance(&s0, s2, &s1, EPSILON, isovalue, w0); 	
        ...
        int visualize_variance = 1;
     
        //visualize variance
        if(visualize_variance == 1)
        { 
     	...
     	else if(visualize_variance == 0)
    	{ 
    	 	 double lin = (double)((s1 - isovalue) / s_diff);
    	
    		 double diff_lin_w0  = fabs(lin - w0);
    		
    	 	 // find maximum difference
      		if(diff_lin_w0 > *max_variance)
			*max_variance = diff_lin_w0;
				
		// store unnormalized difference	
      			*color = diff_lin_w0;	     
    ...
    // if(s2>0.5 || s3>0.5) 
    //cout<<"s2:"<<(double)s2<<" s3:"<<(double)s3<<"\n";   	
    linear_interpolate_coord
      (dimension, w0, coord0, coord1, coord2);	
